...
void TIMG12_IRQHandler(void){uint32_t pos;
  ...
  GPIOB->DOUTTGL31_0 = GREEN;       // toggle PB27 (minimally intrusive debugging)
  Position = Sensor.Convert(Data);  // convert to fixed point distance
 
  IRxmt_OutChar('<');
  data1 = Position/1000;
  IRxmt_OutChar(data1+0x30);
  data2 = Position/100 - data1*10; 
  IRxmt_OutChar(data2+0x30);
  data3 = (Position%100 - Position%10)/10;
  IRxmt_OutChar(data3+0x30);

  
  // for(int i=0; i<3; i++) {            // output 4-frame message
  //   IRxmt_OutChar(Position%16);
  //   Position /= 16;
  // }
    GPIOB->DOUTTGL31_0 = GREEN;     // toggle PB27 (minimally intrusive debugging)
  }
...
int main(void){ // main5T
  ...
  TimerG12_IntArm(2666667,2); // 80MHz/8000 = 100Hz
  ST7735_InitPrintf(INITR_BLACKTAB);
  __enable_irq();

  while(1){
    // ST7735_SetCursor(0, 0);
    // ST7735_OutChar(data1+0x30);
    // ST7735_OutChar('.');
...
int main(void){ // main5R
  ...
  while(1){
    ...
    bool tf;
      ...
      ones = UART2_InChar();
      tenths = UART2_InChar();
      hundreths = UART2_InChar();
      } while ((ones < 0x30 || ones > 0x39) || (tenths < 0x30 || tenths > 0x39) || (hundreths < 0x30 || hundreths > 0x39 ));
      ...
      ST7735_OutChar(ones);
      ST7735_OutChar(0x2E);
      ST7735_OutChar(tenths);
      ST7735_OutChar(hundreths);
