/* IRxmt.cpp
 * Arianna Lauren Pascual, Ronit Verma
 * Date: 7 April 2026
...
#define PulsePerBit 2105 // 16 pulses each bit
...
void IRxmt_Init(void){
    IOMUX->SECCFG.PINCM[PA8INDEX] = 0x081;          // initialize PA8 as output
    GPIOA->DOE31_0 |= IR;
    GPIOA->DOUT31_0 = (GPIOA->DOUT31_0&(~IR));      // make PA8 low (IR off)   
}
...
void IRxmt_SendBit(int bit){
   // write this
    if (bit) {
        ...
        // Delay(16*IRPULSE);
        for(int i=0;i<16;i++) {
            GPIOA->DOUT31_0 = (GPIOA->DOUT31_0&(~IR));
            Delay(IRPULSE);
            GPIOA->DOUT31_0 = (GPIOA->DOUT31_0&(~IR));
            Delay(IRPULSE);
        }
